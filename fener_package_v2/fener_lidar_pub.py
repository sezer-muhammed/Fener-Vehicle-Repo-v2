...
class lidar_publisher(Node):
    def __init__(self):
        ...
        self.declare_parameter("range_max", 4.3)
    ...
    def timer_callback(self):
        ...
        for i, scan in enumerate(self.lidar.iter_scans()):
            ...
            self.laser_msg.angle_min = 0.0 + self.bias
            self.laser_msg.angle_max = 2 * np.pi + self.bias 
            self.laser_msg.ranges = (np.flip(scan[:, 1]) / 1000).tolist()
